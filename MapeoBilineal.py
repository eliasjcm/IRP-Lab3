...
new_img = newImageInterpolation8(img_gray, 2.1+2.1j, 0, 0.003, 1+1j)
# new_img = newImage(img_gray, 2.1+2.1j, 0, 0.003, 1+1j)

# mapeo = crear_mapeo_inverso(2.1+2.1j, 0, 0.003, 1+1j)
# print(f"({410}, {274}) -> ({int(mapeo(complex(410, 274)).real)}, {int(mapeo(complex(410, 274)).imag)})")

blur = addGaussianBlur(new_img, 5)

plt.imshow(blur, cmap='gray')
plt.show()

cv2.imwrite("imagen8.png", blur)